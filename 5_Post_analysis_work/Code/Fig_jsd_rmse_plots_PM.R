...
ggplot(delta.jsd.median, aes(scene, value, colour=variable)) +
  geom_point()  + 
  geom_smooth(aes(scene, value, colour=variable), method=lm, se=FALSE) +
  facet_wrap(~ variable, nrow = 1, scales="free_x") +
  labs(x = "# missing cell types", y = expression(Delta*"JSD (median)")) + 
...
ggplot(delta.rmse.median, aes(scene, value, colour=variable)) +
  geom_point()  + 
  geom_smooth(aes(scene, value, colour=variable), method=lm, se=FALSE) +
  facet_wrap(~ variable, nrow = 1, scales="free_x") +
