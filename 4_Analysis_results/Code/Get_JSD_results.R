...
# 
# 
# JSD.mean.1_ST.rds, JSD.mean.2_ST.rds, JSD.mean.3_ST.rds
...
jsdList <- calculate_JSD(1, st_num, Method_Res)

# mean JSD value for a method across all the spots
j.list[[counter_]] <- jsdList[[1]]
counter_ <- counter_ + 1
...
if ("rm1" %in% rm_scenarios) {
  ...
  jsdList <- calculate_JSD(13, st_num, Method_Res)
...
if ("rm2" %in% rm_scenarios) {
  ...
  jsdList <- calculate_JSD(5, st_num, Method_Res)
...
saveRDS(j.list, paste0(Results, "JSD.means.", st_num,"_ST.rds"))
