...
It is advised to follow the steps mentioned below, to make sure all the reqired paths for packages are set up correctly. <br>
<br>
...
* For reproducibility, the project utilises conda and renv functionality. <br>
	...
	**I**. Please install R version 4.1.2 (archived R source files cam be found [here](https://cran.rstudio.com/src/base/R-4/)).
	
	**II**. 	<font color="brown">[optional]</font> Read [<font color="blue">getting started with renv</font>](https://rstudio.github.io/renv/articles/renv.html) carefully to understand the functionalities provided by the renv R package.
		
	**III**. Executing `Renv_Setup.R` from where it resides currently to initialise the renv infrastructure for the R project (ignore the warning messages):
	...
	&emsp;&emsp;&emsp;&emsp;
*The required files for renv setup <font color="brown">(renv.lock, .Rprofile, renv/activate.R, renv/settings.json)</font> should already be in your R project directory; If not, ensure it resides in the same directory as the Renv_setup.R file.*
...
* Generating a single-cell reference dataset from multiple single-cell datasets, UMAP representations included in the supplementary information of the manuscript using the commands as below;
	...
	> <font color="red">Note:</font> this script takes significant amount of time (~2-3 hours on machine with 16GB RAM) to execute. Ignore the warnings during the execution
...
* Generating single-cell reference datasets for all removal scenarios based on cell type removal using the commands as below;
	...
	If you get python error, please see the known issues.
...
* Generating simulated spatial transcriptomics datasets using single-cell reference data, the three datasets vary by the number of cells and cell types present per spatial location using the commands as below;
	...
	>	&emsp;&emsp;&emsp;&emsp;arg5 - index of simulated ST dataset [*options: 1, 2, 3* ] <br>
		 ...
		 To reproduce the results in the manuscript the following provides the command line input to generate 1st simulated ST dataset,
...
OR	<br>
	...
	Please make sure to execute only one script at a time and wait for the results. For every removal scenario, each single-cell reference will have one result (for instance, rm1 will have 13 results for each method) for every method.
	 ...
	 If your machine crashes, please adapt the shell script accordingly (for instance, starts less jobs in parallel). Also, verify the number of results generated before moving to the next scenario. If a combination of SC & ST dataset is failed for a method, please run the standalone script for that particular pair. <br>
	...
	*The standalone scripts for each deconvolution method expect the command line arguments as below; notice the different versions of argument <font color="red">b </font>*.
	...
	Known failed instances of deconvolution methods: CARD for all 5 rm11, Seurat for 3 rm10, Seurat for 2 rm11
...
* Generating simulated spatial transcriptomics datasets using single-cell reference data, the three datasets vary by the number of cells and cell types present per spatial location using the commands as below;
	...
	>	&emsp;&emsp;&emsp;&emsp;arg5 - index of simulated ST dataset [*options: 1, 2, 3* ] <br>
		...
		To reproduce the results in the manuscript use command line input to generate 1st simulated ST dataset as below,
		Rscript Generate_ST_data.R 4 8 10 15 1
...
* The shell/batch scripts execute all deconvolution methods to predict cell type proportions simultaneously for all removal scenarios and multiple reference datasets for each scenario. <br>
	...
	> &emsp;&emsp;&emsp;&emsp;arg3 - removal scenario [*options: rm0, rm1, rm2, rm3, rm5, rm10, rm11* ]
		...
		To reproduce the results in the manuscript use command line input for no cell type removal (baseline) & one or more cell type removal scenarios as follows,
		(1) ./Execute_R_based_methods.sh 1 1 rm0
		(2) ./Execute_R_based_methods.sh 1 13 rm1
		(3) ./Execute_R_based_methods.sh 1 5 rm2
		(4) ./Execute_R_based_methods.sh 1 5 rm3
...
* Estimating performance metrics for all the removal of cell types scenarios.
	...
	&emsp;&emsp;&emsp;&emsp;arg3 - [*optional; default is all methods*] name of the deconvolution methods separated by comma <br>
		...
		To reproduce the results in the manuscript use command line input for calculating JSD metrics for all methods with 1st simulated ST data and all removal of cell type scenarios as follow,
		Rscript Get_JSD_results.R 1
   ...
   > &emsp;&emsp;&emsp;&emsp;<font color="red">Note: </font> The required command line arguments for <font color="blue">`Get_RMSE_results.R`</font>script are as below:<br>
	...
	&emsp;&emsp;&emsp;&emsp;arg3 - [*optional; default is all methods* ] name of the deconvolution methods separated by comma <br>
		...
		To reproduce the results in the manuscript use command line input for calculating RMSE metrics for all methods with 1st simulated ST data and all removal of cell type scenarios as follow,
		Rscript Get_RMSE_results.R 1
   ...
   - **For cell type reassignment calculations:** <br>
	...
	> &emsp;&emsp;&emsp;&emsp;<font color="red">Note: </font> The required command line arguments for <font color="blue">`Get_celltype_assignment_results.R`</font> script are as below:<br>
	&emsp;&emsp;&emsp;&emsp;arg1 - index of the simulated ST dataset [*options: 1, 2, 3* ] <br>
	&emsp;&emsp;&emsp;&emsp;arg2 - (list) removal scenarios separated by comma (*rm0* is included by default) [*options: rm1, rm2, rm3* ] <br>
	&emsp;&emsp;&emsp;&emsp;arg3 - [*optional; default is all methods* ] name of the deconvolution methods separated by comma <br>
		
		To reproduce the results in the manuscript use command line input for calculating cell type reassignment for all methods with 1st simulated ST data, and removal of one, two & three cell type scenarios as follow,
		Rscript Get_celltype_assignment_results.R 1 "rm1","rm2","rm3"
...
* Generating figures/plots included in the manuscript for calculated JSD, RMSE and cell type reassignment estimates for the specified cell type removal scenarios.
   ...
   - Generate plots for cell type correlation and cell type reassignment plots for cell type removal scenarios rm1, rm2, and rm3 using commands as below;
	...
	&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;arg1 - index of the simulated ST dataset [*options: 1, 2, 3* ] <br>
		...
		To reproduce the results in the manuscript use command line input as below,
		Rscript Celltype_correlation.R
		Rscript Fig_celltype_assignments.R 1
...
#### 6. python_config_impl(python) error

The R scripts uses installed python using RETICULATE package. If the pacakge cannot find the proper python installation, you get this error.

The issue can be resolved by adding below line at the beginning of the R script that is giving the error, to set up the path for RETICULATE_python.
